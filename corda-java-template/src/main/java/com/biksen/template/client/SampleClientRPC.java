...
import com.biksen.template.state.SampleState;
import kotlin.Pair;
...
public class SampleClientRPC {
    public static void main(String[] args) throws ActiveMQException, InterruptedException, ExecutionException {
        ...
        final Logger logger = LoggerFactory.getLogger(SampleClientRPC.class);
        ...
        futureTxs.startWith(txs).toBlocking().subscribe(
                transaction ->
                        transaction.getTx().getOutputs().forEach(
                                output -> {
                                    final SampleState templateState = (SampleState) output.getData();
